...
var defaultBankDirectoryEntries = mustLoadDefaultBankDirectory()

var popularBankEntries = []*domain.BankDirectoryEntry{
...
var supplementalBankEntries = []*domain.BankDirectoryEntry{
	{BankCode: "039", BankName: "Stanbic IBTC Bank"},
	{BankCode: "305", BankName: "OPay Digital Services Limited (OPay)"},
	{BankCode: "796", BankName: "Moniepoint MFB"},
	{BankCode: "855", BankName: "PalmPay"},
	{BankCode: "100004", BankName: "OPay Digital Services Limited (OPay)"},
}

var popularBankOrder = map[string]int{
	"058": 0,
	"044": 1,
	"033": 2,
	"011": 3,
...
const nigerianBankDirectoryJSON = `{
  "data": [
    ...
    {"name": "KONGAPAY (Kongapay Technologies Limited)(formerly Zinternet)", "code": "100025"},
...
func defaultBankDirectory() []*domain.BankDirectoryEntry {
	return cloneBankDirectoryEntries(defaultBankDirectoryEntries)
}

func mergeBankDirectories(primary []*domain.BankDirectoryEntry, secondary []*domain.BankDirectoryEntry) []*domain.BankDirectoryEntry {
	merged := make(map[string]*domain.BankDirectoryEntry)

	upsert := func(entry *domain.BankDirectoryEntry, allowNameOverride bool) {
		if entry == nil {
			return
		}

		code := normalizeBankDirectoryCode(entry.BankCode)
		if code == "" {
			return
		}
		...
		} else if allowNameOverride && name != "" && shouldOverrideBankName(existing.BankName, name) {
			existing.BankName = name
		}
	}
	...
	results := make([]*domain.BankDirectoryEntry, 0, len(merged))
	for _, entry := range merged {
		results = append(results, entry)
	}

	sort.Slice(results, func(i, j int) bool {
		left := normalizeBankDirectoryCode(results[i].BankCode)
		right := normalizeBankDirectoryCode(results[j].BankCode)
		leftRank, leftPopular := popularBankOrder[left]
		rightRank, rightPopular := popularBankOrder[right]
		switch {
		case leftPopular && rightPopular:
		...
		leftName := normalizeBankDirectoryName(results[i].BankName)
		rightName := normalizeBankDirectoryName(results[j].BankName)
...
func lookupBankByCode(code string) *domain.BankDirectoryEntry {
	normalizedCode := normalizeBankDirectoryCode(code)
	if normalizedCode == "" {
		return nil
	}

	for _, entry := range defaultBankDirectoryEntries {
		if normalizeBankDirectoryCode(entry.BankCode) == normalizedCode {
			copyEntry := *entry
			return &copyEntry
		}
	}
...
func mustLoadDefaultBankDirectory() []*domain.BankDirectoryEntry {
	var payload struct {
		Data []struct {
	...
	if err := json.Unmarshal([]byte(nigerianBankDirectoryJSON), &payload); err != nil {
		panic(fmt.Sprintf("failed to load nigerian bank directory: %v", err))
	}

	entries := make([]*domain.BankDirectoryEntry, 0, len(payload.Data)+len(supplementalBankEntries))
	for _, bank := range payload.Data {
		entries = append(entries, &domain.BankDirectoryEntry{
			BankCode: bank.Code,
			BankName: bank.Name,
		})
	}

	combined := append(entries, supplementalBankEntries...)
	return mergeBankDirectories(popularBankEntries, combined)
}

func cloneBankDirectoryEntries(entries []*domain.BankDirectoryEntry) []*domain.BankDirectoryEntry {
	cloned := make([]*domain.BankDirectoryEntry, 0, len(entries))
...
func normalizeBankDirectoryCode(code string) string {
	return strings.TrimSpace(code)
}

func normalizeBankDirectoryName(name string) string {
	return strings.Join(strings.Fields(strings.TrimSpace(name)), " ")
}

func shouldOverrideBankName(existing string, incoming string) bool {
	existing = normalizeBankDirectoryName(existing)
	incoming = normalizeBankDirectoryName(incoming)
	switch {
	case incoming == "":
		return false
	case existing == "":
	...
	case strings.EqualFold(existing, incoming):
		return false
	case strings.EqualFold(existing, "Nigerian Bank"):
		return true
	case len(incoming) > len(existing) && strings.Contains(strings.ToUpper(incoming), strings.ToUpper(existing)):
		return true
	default:
		return false
	}
